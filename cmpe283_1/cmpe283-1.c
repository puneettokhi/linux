...
/*
 *Defining entry number for each control field 
 */
#define PINBASED_CTLS_NUM     5
#define PROCBASED_CTLS_NUM    22
#define PROCBASED_CTLS2_NUM   27
#define EXIT_CTLS_NUM         16
#define ENTRY_CTLS_NUM        13
 ...
 */
struct capability_info pinbased[PINBASED_CTLS_NUM] =
{
 ...
 */
struct capability_info procbased[PROCBASED_CTLS_NUM] =
{
 ...
 */
struct capability_info procbased2[PROCBASED_CTLS2_NUM] =
{
...
/*
 * Exit control field capabilities
 * See SDM volume 3, section 24.7.1 Table 24-13
 */
struct capability_info exitctl[EXIT_CTLS_NUM] =
{
...
/*
 * Entry control field capabilities
 * See SDM volume 3, section 24.8.1 Table 24-15
 */

struct capability_info entryctl[ENTRY_CTLS_NUM] =
{
...
detect_vmx_features(void)
{
	...
	report_capability(pinbased, PINBASED_CTLS_NUM, lo, hi);
	...
	pr_info("Procbased Primary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased, PROCBASED_CTLS_NUM, lo, hi);

	/* Procbased2 controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Procbased Secondary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased2, PROCBASED_CTLS2_NUM, lo, hi);

	/* Exit controls */
	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exitctl, EXIT_CTLS_NUM, lo, hi);

	/* Entry controls */
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entryctl, ENTRY_CTLS_NUM, lo, hi);
